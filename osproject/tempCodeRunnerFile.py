...
class VirtualMemoryManager:
    def __init__(self, num_segments, segment_sizes):
        ...
        self.memory_blocks = [{'segment_number': i+1, 'start_address': i, 'size': size, 'status': 'free'} for i, size in enumerate(segment_sizes)]
    ...
    def allocate_process(self, process_id, process_size, allocation_strategy):
        ...
        if allocation_result:
            self.allocated_processes.append({
                ...
                'segment_number': allocation_result['segment_number'],
                'start_address': allocation_result['start_address'],
                'end_address': allocation_result['start_address'] + allocation_result['size'],
                'allocation_strategy': allocation_strategy
            })

            print(f"Process P{process_id} allocated successfully:")
            print(f"Segment Number: {allocation_result['segment_number']}")
            print(f"Start Address: {allocation_result['start_address']}")
            print(f"End Address: {allocation_result['start_address'] + allocation_result['size']}")
    ...
    def deallocate(self, process_id):
        for block in self.memory_blocks:
            if block.get('process_id') == process_id and block['status'] == 'occupied':
                ...
                print(f"Process P{process_id} deallocated successfully.")
                print("------------------------")
    ...
    def get_process_details_table(self):
        ...
        for process_info in self.allocated_processes:
            data.append({
                ...
                'Segment Number': process_info['segment_number'],
    ...
    def display_memory_layout(self):
        ...
        for block in self.memory_blocks:
            ...
            ax.text(1, block['start_address'] + block['size'] / 2, f"Segment {block['segment_number']}\nSize: {block['size']}\nStatus: {block['status']}", va='center')
